...
func TestBase(t *testing.T) {
	...
	evalResult1, err := transactionOut.ApplyOperator(NewAvgTicketAnalyzer("avg ticket analyzer", 2, NewUserAccountStrategy()))

	evalResult2, err := transactionOut.ApplyOperator(NewWindowedProximityAnalyzer("windowed proximity analyzer", 2, NewUserAccountStrategy()))

	evalResult3, err := transactionOut.ApplyOperator(NewWindowedTransactionCountAnalyzer("windowed transaction count analyzer", 2, NewUserAccountStrategy()))

	stream.Of(evalResult1, evalResult2, evalResult3).ApplyOperator(NewScoreAggregator("score aggregator", 2, NewTranIdFieldStrategy()))
